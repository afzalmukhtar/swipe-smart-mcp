...
class FinancialAnalytics:
    ...
    async def calculate_rewards(self, expense: Expense) -> RewardCalculation | None:
        ...
        for bonus_cat, rate in card.bonus_categories.items():
            if (
                bonus_cat.lower() in category_lower
                or category_lower in bonus_cat.lower()
            ):
                reward_rate = rate
                break
        ...
        if reward_rate == 0.0:
            for reward_cat, rate in card.reward_categories.items():
                if (
                    reward_cat.lower() in category_lower
                    or category_lower in reward_cat.lower()
                ):
                    reward_rate = rate
                    break
    ...
    async def get_best_card_for_category(
        self, category: str, amount: float = 100
    ) -> dict[str, Any] | None:
    ...
    async def get_financial_summary(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        for expense in expenses:
            ...
            payment_totals[method] = (
                payment_totals.get(method, Decimal("0")) + expense.amount
            )
        ...
        return {
            ...
            "payment_methods": [
                (method, float(amount)) for method, amount in payment_methods
            ],
    ...
    async def _analyze_categories(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if top_categories:
            ...
            if len(top_categories) > 1:
                insights.append(
                    f"Top 3 categories account for {len(top_categories[:3])} spending areas"
                )
    ...
    async def _analyze_cards(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if cards:
            ...
            insights.append(
                "Consider using cards with highest reward rates for each category"
            )
        else:
    ...
    async def _analyze_rewards(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        for expense in expenses:
            ...
            if expense.category:
                best_card = await self.get_best_card_for_category(
                    expense.category, float(expense.amount)
                )
                if best_card:
                    potential_reward = (
                        expense.amount * Decimal(best_card["reward_rate"]) / 100
                    )
                    if not current_reward or potential_reward > current_reward.amount:
                        missed_rewards += potential_reward - (
                            current_reward.amount if current_reward else Decimal("0")
                        )
